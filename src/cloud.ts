 ...
 *
 * cloud.ts — Capa de nube (Supabase) para Pizza NYC (molde CyC).
 * Base compartida: validar_licencia / reclamar_tienda / sincronizar_clave_dueno
 * / tl_miembros. Datos del local en la tabla pizza_backups (jsonb "datos").
 * Prefijo de la app: pizza_*.
 */
...
export async function asegurarCuentaSeguraColab(usuario: string, password: string, codigo: string) {
  ...
  if (!sess) {
    let ok = false;
    try { ok = await rpc('pizza_verificar_colab', { p_codigo: codigo, p_usuario: usuario, p_pass: password }, false); }
    catch (e) { ok = false; }
  ...
  try { await rpc('pizza_unir_colab', { p_codigo: codigo, p_usuario: usuario }); }
...
// ── Sync de datos del local (tabla pizza_backups, RLS por membresía) ────
export async function cloudLoad(codigo: string): Promise<CloudData | null> {
  const bearer = (await authToken()) || SB_KEY;
  try {
    const res = await fetch(
      `${SB_URL}/rest/v1/pizza_backups?tenant_id=eq.${encodeURIComponent(codigo)}&select=datos&limit=1`,
...
export async function cloudSave(codigo: string, datos: CloudData): Promise<boolean> {
  const bearer = (await authToken()) || SB_KEY;
  try {
    const res = await fetch(`${SB_URL}/rest/v1/pizza_backups`, {
      method: 'POST',
      headers: {
...
// ── Público (catálogo) y alta de pedido/opinión ───────────────────────
export async function pizzaPublica(codigo: string): Promise<CloudData | null> {
  try { return await rpc('pizza_publica', { p_codigo: codigo }, false) as CloudData; }
  catch (e) { return null; }
}

export async function pizzaAgregarPedido(codigo: string, pedido: any): Promise<void> {
  try { await rpc('pizza_agregar_pedido', { p_codigo: codigo, p_pedido: pedido }, false); }
  catch (e) { /* noop */ }
}

export async function pizzaAgregarResena(codigo: string, comment: any): Promise<void> {
  try { await rpc('pizza_agregar_resena', { p_codigo: codigo, p_comment: comment }, false); }
  catch (e) { /* noop */ }
}

export async function pizzaVersion(codigo: string): Promise<string> {
